...
> Last updated: 2026-09-18 (ADR-064 — Instructions-first onboarding pattern for games, rolled out to all 101 games)
...
## ADR-062 — Game sessions never closed on exit/quit/tab-close (only on win)

**Date:** 2026-07-14
**Status:** Accepted

**Decision:** Fix `game_sessions` rows staying `status = 'active'` forever (200+ stuck rows observed on the live AWS RDS dashboard) by ending the session on every real exit path, not just the happy path. Root cause was **not** missing game-over detection — all 9 gesture games already posted `GAME_COMPLETE` on their actual terminal state — it was that `endSession()` had exactly one call site in the whole app (`handleGameComplete` in the play shell), so every other way a session actually ends (Exit button, tab close, browser crash, background/hide, or `gesture-love-balls` specifically only reporting a **win**, never a loss or quit) left the row open indefinitely.

**Fix applied at the shell level (`src/app/(play)/games/[id]/play/page.tsx`), not per-game**, since the games can't be trusted individually to cover every exit path: a `sessionEndedRef` guard plus a shared `endSessionOnce()` wired to (1) the Exit button's click handler, (2) the session-start effect's unmount cleanup, and (3) `beforeunload`/`visibilitychange` listeners using `fetch(..., { method: "PATCH", keepalive: true })` — `keepalive` (not `sendBeacon`, which can't do PATCH) is what lets the request survive page teardown on tab close.

**`gesture-love-balls` separately fixed to post `GAME_COMPLETE` on level failure and on explicit Quit**, not only on win — previously the only game in the catalog whose own internal win/lose logic didn't even attempt to report a loss.

**One-time data cleanup of the pre-existing 200+ stuck rows was handed to the user as a plain SQL statement** (`UPDATE game_sessions SET status='ended', ended_at=GREATEST(now(), started_at + interval '1 second') WHERE status='active'`) to run themselves via their DB GUI against AWS RDS — consistent with the migration-access gap in ADR-060 (the app's DB role can self-`UPDATE` fine, this wasn't a privilege issue, just a one-off data fix not worth a migration file for).

**Files modified:** `src/app/(play)/games/[id]/play/page.tsx`; `games/gesture-love-balls/src/App.tsx` (fail-check + `handleQuit` wrapper posting `GAME_COMPLETE` with score 0).

---

## ADR-063 — Frontend polish: favorites/wishlist, persisted display settings, per-rail "View all"

**Date:** 2026-07-14
**Status:** Accepted

**Decision:** Ship three independent items from the Phase 1.x frontend-polish backlog in one pass, user-selected out of a longer list (activating real cover art was explicitly left out — it's a manual asset-upload task, not code).

**Favorites/wishlist** is new: a `favorites` table (migration 0011, cloned from `game_ratings`'s `(user_id, game_id)` unique-composite-key shape) + `favorites-repository.ts` (`add`/`remove`/`list`) + `/api/favorites` (GET/POST) + `/api/favorites/[gameId]` (DELETE), all following `sessions-repository.ts`/`/api/sessions`'s existing auth-via-`getCurrentUser()` pattern exactly. Client side is a **shared singleton hook** (`useFavorites`, module-level `Set<string>` + listener pub/sub, same shape as `useGames`/`useContinuePlaying`'s module-level cache) rather than per-instance state, so every `GameCard`'s heart button across a page (dashboard + library simultaneously) reflects the same toggle instantly with no prop drilling; toggling is optimistic with rollback-on-failure. The heart button (`FavoriteButton`) only renders when `useSession().isAuthenticated` is true — the dashboard and library are publicly browsable (not in `proxy.ts`'s `PROTECTED_PREFIXES`), so gating client-side avoids ever hitting the API as a logged-out visitor. `invalidateFavorites()` is called on `logout`/`logoutAll` in `session-context.tsx` to prevent one account's cached favorites leaking into a next sign-in on the same tab.

**Persisted display settings** turned out to require fixing a second, related gap: `useSettings` previously only applied its `data-large-text`/`data-reduce-motion` attributes to `<html>` when the Settings page itself happened to mount — a hard reload of any other page without ever visiting `/settings` first got no accessibility styling at all this session. Fixed by converting `useSettings` into the same shared-singleton pattern as `useFavorites`, and mounting a new invisible `<DisplaySettingsSync />` component once in the root layout (inside `SessionProvider`, alongside `SpatialNavigation`) so settings apply globally on every page load. Server-side persistence reuses the **existing** `PATCH /api/auth/me` endpoint rather than a new route — `large_text`/`reduce_motion` columns added to `users` (migration 0012) and threaded through the whole existing profile-update pipeline (`UserRepository.update()`'s already-generic dynamic-`SET`-clause implementation needed zero logic changes, just a widened TS Partial type; `toAuthUser`/`AuthUser`/`UserRecord`/UI `User`/`toAppUser` all extended in parallel). On login, the user's server-persisted values sync into the shared singleton once (`syncedFromServer` module flag) so they follow the account across devices/browsers, while `localStorage` remains the fast local cache and the only source of truth for logged-out visitors.

**Per-rail "View all" links** were mostly wiring `GameRail`'s existing (unused-by-2-of-3-rails) `subtitle`/`viewAllHref` props: Featured games now links to `/library?featured=1`, All games to plain `/library`. Library page gained a `featured=1` query-param filter (intersected with the existing search/category filters) since no such filter existed before. **Continue Playing was deliberately left without a "View all" link** — there's no meaningful full-page equivalent to deep-link into (it's not a category, and a generic `/library` link would be misleading about what it shows).

**Migration-access note (see ADR-060):** both migrations 0011 and 0012 need the user to run them manually via their DB GUI against AWS RDS, same as every migration since 0006 — `kintofun_user` still lacks `CREATE`/`ALTER` privilege on `public`.

**Files added:** `supabase/migrations/{0011_favorites,0012_user_display_settings}.sql`; `src/lib/data/favorites-repository.ts`; `src/app/api/favorites/route.ts` + `[gameId]/route.ts`; `src/services/favorites.service.ts`; `src/features/games/useFavorites.ts`; `src/components/game/FavoriteButton.tsx`; `src/features/settings/DisplaySettingsSync.tsx`. **Files modified:** `src/components/game/GameCard.tsx` (heart button); `src/features/auth/session-context.tsx` (`invalidateFavorites` on logout); `src/types/auth.ts`, `src/lib/auth/serialize.ts`, `src/lib/auth/repository.ts`, `src/lib/auth/repositories/{pg,local}-user-repository.ts`, `src/app/api/auth/me/route.ts`, `src/types/index.ts`, `src/services/auth.service.ts` (display-settings plumbing); `src/features/settings/useSettings.ts` (rewritten as shared singleton); `src/app/layout.tsx` (mounts `DisplaySettingsSync`); `src/app/(app)/page.tsx`, `src/app/(app)/library/page.tsx` (View all links + featured filter).

**Not yet verified live:** no browser/webcam pass was run against these three features this session (text-only implementation + `tsc --noEmit` clean on every touched file) — recommend clicking through favorites toggle, a logout/login cycle, and the Settings toggles in an actual browser before considering this fully done, per the platform's usual "typecheck isn't feature verification" standard.

**Follow-up fix [2026-07-14], found immediately by the user clicking the new heart button in a real browser — exactly the live-verification gap flagged above:** the heart button never actually toggled; clicking it always navigated to the game detail page instead. Root cause: `GameCard`'s decorative "centre play button" overlay (`absolute inset-0 z-20`, spanning the *entire* card) sits later in the DOM than the top badge row (also `z-20`) — equal `z-index` resolves by DOM order, so this full-card div painted on top of everything below it, including the `FavoriteButton`, and its default `pointer-events: auto` silently absorbed every click on the card face and let it bubble to the enclosing `<Link>`. Harmless before the heart button existed (the whole card was supposed to navigate anyway), but it meant no element placed inside the card could ever be independently clickable. Fixed with a single `pointer-events-none` on that overlay div. **Also added a "Favorites" dashboard rail** (`src/app/(app)/page.tsx`, right after Continue Playing) since favoriting had no way to view the list anywhere — reuses `useFavorites()`'s existing singleton cache, no new API calls. One implementation note worth keeping in mind for future favorites-consuming code: `useFavorites().isFavorite` is a referentially-stable callback closing over a module-level `Set` — a `useMemo` keyed on it will **not** recompute when favorites actually change (only `useFavorites()`'s own internal re-render signal fires); the dashboard rail therefore computes `favoriteGames` as a plain (unmemoized) filter each render rather than memoizing it.

---

## ADR-064 — "Instructions-first" onboarding pattern for games (reference implementation: Gesture Piano)

**Date:** 2026-09-18
**Status:** Accepted (reference pattern — rollout to the other ~60 games is a separate, per-game task, tracked in the Roadmap)

**Decision:** Every game must show its own How-to-Play instructions **before the player can start playing**, not just on-demand behind a button — per explicit platform requirement. Gesture Piano (`games/gesture-piano/`) was used to work out the pattern live with the user across several iterations; this ADR documents the resulting shape so it can be repeated on the rest of the catalog without re-deriving it, while keeping each game's actual instructional *content* game-specific (control scheme, scoring, modes — these differ enormously across the catalog and must never be copy-pasted from Piano's text).

**Two-layer pattern, both required:**
1. **App-entry layer** (`App.tsx`'s top-level `Screen` state machine): the game still opens on its normal landing screen (title/art/Play button) for a short **`INTRO_DELAY_MS`** (2200ms here), then **auto-transitions to the existing `HowToPlayScreen`** via a `useEffect(() => setTimeout(...), [])` that only fires if the player hasn't already navigated away on their own (checked with a functional `setScreen(s => s === 'landing' ? 'howtoplay' : s)`, not a bare `setScreen('howtoplay')`, so an eager click isn't overridden). The How-to-Play screen is dismissed by a single primary **"Continue"** button. **The landing screen's own "How to Play" button is left wired exactly as before**, so the same screen is reachable on demand later too — this is one shared screen serving both "forced first look" and "let me re-check the rules" duty, not two separate implementations.
2. **Pre-round layer** (inside the actual game screen, after mode/level select, before gameplay begins): a new **`'ready'` phase** added to the game's state machine (`GameState.phase: 'ready' | 'playing' | ...`), during which the game clock/timer is **frozen** (`stepGame` returns state unchanged while `phase === 'ready'`) and an overlay shows **mode-specific** copy (a small `Record<GameMode, {title, desc}>` map) plus an explicit **"Start Playing"** button. This is the layer that actually matters for a *gesture*-controlled game: relying on a gesture (e.g. "lower a fingertip to start") to dismiss it is a trap — a hand already resting in the trigger zone when the camera warms up satisfies "first tracked frame counts as a fresh press" instantly, skipping the overlay before it's readable. **A plain clickable button, independent of hand-tracking/timing, is the only fully deterministic dismissal.** (A `MIN_READY_MS` "must be visibly on screen for Nms before a gesture-press counts" guard was tried first as a softer fix; it reduced but did not eliminate the failure mode, and the explicit button is what actually solved the user's repeated "still nothing" reports — keep the button as the primary mechanism, not a fallback.)

**Explicitly rejected mid-iteration, worth not re-adding elsewhere:** a small ✕ close icon on the instructions screen (tried as an alternative/additional dismissal control, first floating over the whole screen — which visually collided with the centered title — then repositioned into the title row itself). The user ultimately asked to remove it entirely and keep dismissal to the single "Continue" button only. Treat "one primary Continue action, no secondary close icon" as the default for this instructions screen pattern unless a specific game/user asks otherwise.

**Rollout guidance for the remaining games:** apply the same two-layer structure (auto-opening entry instructions + mode-specific frozen "ready" screen with an explicit Start button), but (a) write fresh instruction bullets from that game's actual mechanics — swipe/pinch/wrist-tilt/two-hand games all read completely differently from Piano's fingertip-zone explanation — and (b) reuse each game's existing menu-button styling/gesture-dwell affordances rather than importing Piano's specific components. Build+sync per game exactly like every other game-catalog change: `npm run build` inside `games/<name>/`, then `npm run build:games <name>` from the repo root to copy into `public/games/<name>/` (gitignored build output — only the `games/<name>/src` changes need committing).

**Files modified (Gesture Piano only, this ADR):** `games/gesture-piano/src/gameLogic.ts` (`'ready'` phase, frozen-clock branch in `stepGame`); `games/gesture-piano/src/App.tsx` (`INTRO_DELAY_MS` auto-transition, `MODE_INTRO` copy, ready-phase overlay + `handleStart`, `MIN_READY_MS` guard, tried-then-reverted ✕ button).

---

**Follow-up [2026-09-18] — rollout completed across all 101 games in `src/games/registry.ts`.** The user explicitly asked for every remaining game, not a sample, so this ADR's rollout guidance above was actually executed end to end, in registry order, reviewing each game's real entry flow (not assuming the pattern applied) before deciding what to change.

**Result: most of the catalog turned out to already be compliant.** Of 101 games, roughly 70 needed no code change at all, because the codebase already had several independent ways of satisfying "show instructions before play" that this ADR's original two-layer description didn't anticipate:
- A **persistent, unconditional on-screen gesture legend** (e.g. `#gesture-hint`, `.legend`, `#ctrl-guide` divs with no `display:none` and no JS toggle) — common across the large "plain HTML / vanilla JS" slice of the catalog (air-painting, block-blast-gesture, gesture-space-shooter, hand-snake-pro, and many more). These never hide, so there's no "before you start" moment to gate at all.
- An **intro/loading screen with instruction cards**, shown automatically before the menu is even interactive (butterfly-catch, catch-the-falling-stars, cookie-monster-game, fish-catcher, whack-a-mole-hand).
- A **mandatory calibration or tutorial step that gates the actual first `startGame()`/play action**, independent of which menu path the player took to get there (gesture-runner, the-sniper-code, spear-stickman, connect-2-balls — the last one via a `localStorage` "seen once" flag rather than a session timer).
- The **"lovable"-template family** (block-breaker, crossword-builder, neon-flow, target-master, word-adventure, mix-it-up, and others sharing that generated project shape) always puts the mechanic explanation directly on the landing route itself — the one page that can't be skipped before reaching `/play` or `/levels` — so no separate gating logic was needed.

**Where a real gap existed, the fix was sized to the gap, not templated from Piano:**
- Most React/Vite games with a `'landing' → 'howtoplay' → ...` (or equivalent) `Screen` union got the exact Piano-style fix: a `useEffect` timer that auto-transitions from the entry screen to the existing How-To-Play screen after ~2.2s, guarded with a functional state update (`setScreen(s => s === 'landing' ? 'howtoplay' : s)`) so a player who already clicked something is never overridden. Games using Zustand instead of local `useState` (gesture-hill-adventure, gesture-snake-arena, gesture-table-tennis, gesture-mob-rally) got the same timer reading/writing the store instead, with a `useRef` one-shot guard for the ones that pass through a calibration screen before ever reaching the menu the first time.
- **world-explorer** had a `Controls` hint box gated on `!gestureState.handPresent` — meaning it vanished at exactly the moment a hand became visible, which is exactly when the player needs it to know how to wave. Fixed by removing that condition, not by adding new UI.
- **mirror-me**'s idle screen explained how to *start* (raise hands / stand still 3s) but never explained the actual round mechanic (copy the pose shown on screen before time runs out) — added that explanation to the same always-shown screen.
- **ninja-star-throw**'s menu tagline ("Point · Aim · Release") was decorative and never named the actual fist-to-aim / open-to-throw gesture, which only ever appeared as a live status label during play — replaced the tagline with a concrete instruction.
- **connect-2-balls** already force-showed a one-time tutorial via a `localStorage` flag but had no button anywhere to reopen it afterward — added a "How to Play" menu tile rather than an auto-open timer, since the auto-open mechanism doesn't apply to a game that's already past its one-time trigger.
- **flag-quest** and a same-template "family" of 12 other games (shape-color-sorter, alphabet-zoo, memory-match-zoo, clock-time-teller, coin-money-counter, weather-seasons-sorter, opposites-match, musical-instrument-sounds, fruit-vegetable-sorter, farm-builder, pocket-pal, circuit-racer, zoo-architect) all share one `MainMenu.tsx` shape (confirmed by their own ADRs' "reuses X's stack near-verbatim" language) ending in a generic "Hover over a tile with your fingertip to select it" line that never explained what a round of *that specific game* actually asks the player to do. flag-quest's calibration screen only ever covered camera positioning, never the paint-fill mechanic itself, and there was no How-To-Play surface anywhere in the game — this one got a genuinely new `HowToPlayScreen.tsx` component (four instruction cards: pick a color, fill the region, wrong-color feedback, how a flag completes) wired to a new `'howto'` screen reached both from a new MainMenu tile and automatically the first time calibration ever completes (calibration only ever runs once per browser, so that moment is a reliable proxy for "this player has never seen instructions"). The other 12 games in the family have one simple, uniform mechanic each ("hover the matching answer to a prompt shown two ways") — for those, a whole new screen was disproportionate; each just got one added sentence on its existing MainMenu describing its own specific prompt/answer shape (e.g. clock-time-teller: "A clock shows a time one way — hover the matching answer"; farm-builder: "Pinch a seed and drop it on a plot to plant it... No fail state — just build your farm!").

**Pre-existing environment issue found and fixed along the way, unrelated to any of these edits:** `games/world-explorer/node_modules` had a truncated `@next/swc-win32-x64-msvc` binary (95KB instead of ~136MB) and a missing `util-deprecate/node.js`, both symptoms of an interrupted `npm install` from before this session. Fixed with `rm -rf node_modules && npm install`. Worth remembering: if a game's `npm run build:games <name>` fails with a webpack/SWC "module not found" or "not a valid Win32 application" error, suspect corrupted `node_modules` from a prior interrupted install before assuming the instructions-first edit broke something.

**Files modified beyond Gesture Piano, by category:**
- Piano-style auto-open timer added to `App.tsx` (or equivalent root component): gesture-tetris, gesture-volleyball, gesture-basketball, gesture-love-balls, gesture-drums, gesture-pong, slingshot-bird, simon-says-hands, traffic-controller, bubble-shooter, pizza-maker, kingdom-clash, gesture-air-hockey, samurai-slice, gesture-hill-adventure, gesture-snake-arena, gesture-table-tennis, gesture-mob-rally, maze-muncher, gesture-road-builder (vanilla JS `MenuSystem.js` constructor), dance-freeze and goalkeeper-hero (vanilla JS, `setTimeout` before the RAF loop start).
- New Next.js game (`dino-runner`): replaced a disabled decorative `<button>` with a real clickable Start/Restart, plus a `MIN_READY_MS`-style guard against a hand already in the jump pose when tracking starts.
- New Next.js game (`mouth-open-catch`): same auto-open timer pattern adapted to its `Screen` union.
- `happy-glass`, `cooking-chef`: smaller additions (a reminder line, a one-shot menu-arrival timer) rather than a full new screen, since their instructions were already mostly automatic.
- `balloon-pop-adventure`: boolean-flag state shape (not a `Screen` enum) — used a `navigatedRef` guard set by the real menu handlers instead of a functional `setScreen` check.
- `flag-quest`: new `HowToPlayScreen.tsx` + `'howto'` screen wiring (see above).
- One-sentence MainMenu additions: shape-color-sorter, alphabet-zoo, memory-match-zoo, clock-time-teller, coin-money-counter, weather-seasons-sorter, opposites-match, musical-instrument-sounds, fruit-vegetable-sorter, farm-builder, pocket-pal, circuit-racer, zoo-architect.
- `world-explorer/app/page.tsx`: removed the `!gestureState.handPresent` condition hiding the Controls hint.
- `mirror-me/src/ui/Renderer.js`: added mechanic explanation to `drawIdleScreen`.
