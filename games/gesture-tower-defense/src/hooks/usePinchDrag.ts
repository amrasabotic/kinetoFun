...
export interface DragPayload {
  source: 'tray' | 'placed';
  typeId: TowerTypeId;
  placedTowerId?: string; // set only when source === 'placed'
}
...
// isPinching is computed fresh per-frame with no smoothing, so a raw pinch
// can flicker for a frame or two on landmark jitter. Require it to sustain
// for this long before treating it as a real grab — the same fix Zoo
// Architect's usePinchDrag (this hook's template) and Gesture Love Balls'
// pinch-to-draw mechanic both use. Release stays instant so letting go
// feels responsive.
...
export function usePinchDrag({ getGrabbables, getDropzones, onDrop, onCancel }: UsePinchDragOptions) {
  ...
  useEffect(() => {
    ...
    const confirmedPinching =
      rawPinching && pinchStartRef.current !== null && performance.now() - pinchStartRef.current >= PINCH_CONFIRM_MS;
