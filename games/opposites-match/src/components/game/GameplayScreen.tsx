...
import type { BinDef, OppositeGameMode, RoundPrompt, SessionResult } from '../../types';
import { generateRound } from '../../game/roundLogic';
import { finalizeSession, type RoundTally } from '../../game/scoring';
import PromptDisplay from './PromptDisplay';
import OppositeBin from './OppositeBin';
...
export default function GameplayScreen({ mode, onExit, onSessionComplete }: Props) {
  ...
  useEffect(() => {
    const next = generateRound(mode, roundIndex, prevPairRef.current);
    prevPairRef.current = next.prompt.pairId;
    setRound(next);
    ...
    if (audioNarration) {
      const text = `What is the opposite of ${next.prompt.word}?`;
  ...
  function handleCorrect(binId: string) {
    ...
    if (audioNarration && round) {
      window.setTimeout(() => speak(`The opposite of ${round.prompt.word} is ${round.prompt.correctWord}!`, true), 200);
    }
  ...
  const binPresentation = round?.prompt.presentation ?? 'word';
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <p className="text-xs text-white/50 uppercase tracking-wide">
            {mode === 'word' ? 'Words' : mode === 'picture' ? 'Pictures' : 'Mixed Mode'}
          </p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-10">
        ...
        <AnimatePresence>
          {wrongThisRound && (
            ...
            >
              Try another opposite — you can do it!
            </motion.p>
          )}
        </AnimatePresence>
