...
const DEFAULT_SAVE: SaveData = {
  version: 1,
  bestStarsByMode: { word: 0, picture: 0, mixed: 0 },
  sessionsPlayed: 0,
  perfectSessions: 0,
  achievements: [],
};
...
function computeNewAchievements(existing: string[], state: SaveData, result: SessionResult): string[] {
  ...
  if (state.bestStarsByMode.word >= 3) add('word-master');
  if (state.bestStarsByMode.picture >= 3) add('picture-master');
