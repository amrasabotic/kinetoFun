...
export interface HandData {
  ...
  detected: boolean;
  confidence: number;
  // Raw landmarks for advanced use (bounding box, calibration outline, etc.)
...
/**
 * Single-hand MediaPipe tracking hook. Returns a MutableRefObject that is
 * updated every frame from the hook's own RAF loop — reading `.current`
 * never triggers a React re-render, so the game loop can poll it at 60fps.
 */
    ...
    async function init() {
      try {
        ...
        const stream = await navigator.mediaDevices.getUserMedia({
          video: deviceId
            ? { deviceId: { exact: deviceId }, width: 960, height: 540 }
        ...
        console.warn('[MobRally] MediaPipe init failed', e);
      }
    }
    ...
    function loop() {
      ...
      try {
        ...
        if (result.landmarks.length > 0) {
          ...
          handRef.current = parseHand(lm, confidence);
        } else {
...
function parseHand(lm: { x: number; y: number; z: number }[], confidence: number): HandData {
