import type { Category, Difficulty, GameMode, PlayerId } from '../types';

const STORAGE_KEY = 'kinetofun.gesture-trivia-arena.save.v1';
...
export function recordMatch(
  ...
  questionsCorrectThisMatch: number,
  categoriesCorrect: Category[],
  isDaily: boolean,
) {
  const save = loadSave();
  save.matchesPlayed += 1;
  if (winner === 'you') save.wins += 1;
  save.questionsAnswered += 9;
  save.questionsCorrect += questionsCorrectThisMatch;
  for (const cat of categoriesCorrect) {
    save.correctByCategory[cat] = (save.correctByCategory[cat] ?? 0) + 1;
  }
