...
const DEFAULT_SAVE: SaveData = {
  version: 1,
  bestStarsByMode: { count: 0, make: 0, mixed: 0 },
  sessionsPlayed: 0,
  perfectSessions: 0,
  achievements: [],
};
...
export const useProgressStore = create<ProgressState>()(
  persist(
    ...
    { name: 'coin-money-counter-progress' },
  ),
);
...
function computeNewAchievements(existing: string[], state: SaveData, result: SessionResult): string[] {
  ...
  if (state.bestStarsByMode.count >= 3) add('coin-counter-master');
  if (state.bestStarsByMode.make >= 3) add('money-maker-master');
