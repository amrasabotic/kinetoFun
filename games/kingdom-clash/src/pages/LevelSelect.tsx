...
const LEVEL_NAMES = [
  "The First Stand", "Goblin Raid", "Orc Invasion", "Troll March", "Dark Advance",
  "Shadow Legion", "Knight's Siege", "Dragon's Breath", "The Last Bastion", "Armageddon",
];

const LEVEL_COLORS = [
  "#44aa44","#55bb44","#aaaa22","#cc8833","#cc5522",
  "#cc3333","#993388","#6633bb","#3344cc","#cc2222",
];

export default function LevelSelect({ cursor, onSelectLevel, onBack }: Props) {
      ...
      <div className="w-full max-w-3xl">
        <div className="flex items-center mb-8 gap-4">
          <DwellButton
            cursor={cursor}
            onActivate={onBack}
            className="px-4 py-2 rounded border text-sm overflow-hidden"
          ...
          {Array.from({ length: TOTAL_LEVELS }, (_, i) => {
            ...
            return (
              <DwellButton
                key={level}
                cursor={cursor}
                onActivate={() => unlocked && onSelectLevel(level)}
                disabled={!unlocked}
                className="flex flex-col items-center gap-2 rounded-xl p-4 overflow-hidden"
                style={{
                  background: unlocked
                    ? `linear-gradient(135deg, rgba(${hexToRgb(color)},0.15), rgba(${hexToRgb(color)},0.05))`
                    : "rgba(20,20,20,0.6)",
                  border: unlocked ? `2px solid ${color}88` : "2px solid #2a2a2a",
                  opacity: unlocked ? 1 : 0.45,
                  boxShadow: unlocked && earned > 0 ? `0 0 16px ${color}33` : "none",
                }}
              >
                <div className="text-3xl font-black" style={{ color: unlocked ? color : "#444" }}>
                  {level}
                </div>
                <div className="text-xs text-center leading-tight" style={{ color: unlocked ? "#ccc" : "#444" }}>
                  {LEVEL_NAMES[i]}
                </div>
                {unlocked ? (
        ...
        <div className="mt-8 text-center text-xs" style={{ color: "#555" }}>
          Hover finger over a level · Hold still to select
        </div>
      </div>
    </div>
  );
