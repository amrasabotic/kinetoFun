...
export default function GameplayScreen({ mode, onExit, onSessionComplete }: Props) {
  ...
  const updateLettersSeen = useProgressStore((s) => s.updateLettersSeen);
  ...
  const prevPromptRef = useRef<RoundPrompt | null>(null);

  useEffect(() => {
    const next = generateRound(roundIndex, prevPromptRef.current);
    prevPromptRef.current = next.prompt;
    setRound(next);
    ...
    const promptLetter = next.prompt.letter;
    updateLettersSeen(promptLetter);

    if (audioNarration) {
      const entry = getLetterEntry(promptLetter);
      if (entry) {
        let text = '';
        if (mode === 'letter') text = `Letter ${promptLetter}`;
        else if (mode === 'animal') text = `${promptLetter} is for ${entry.word}`;
        else text = `${entry.word}, ${promptLetter}`;
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-[#241454] via-[#301a68] to-[#160c38] flex flex-col text-white p-6">
      <div className="flex items-center justify-between mb-2">
        <div>
          <p className="text-xs text-white/50 uppercase tracking-wide">
            {mode === 'letter' ? 'Letter Match' : mode === 'animal' ? 'Animal Sounds' : 'Mixed Mode'}
          </p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-10">
        {round && <PromptDisplay mode={mode} prompt={round.prompt} />}
        ...
        <AnimatePresence>
          {wrongThisRound && (
            ...
            >
              Try another letter — you can do it!
            </motion.p>
          )}
        </AnimatePresence>
        ...
        {round && (
          <div className="flex flex-wrap items-center justify-center gap-6 max-w-3xl">
            {round.bins.map((bin) => (
              ...
              >
                <LetterBin bin={bin} disabled={locked} onCorrect={() => handleCorrect(bin.id)} onWrong={handleWrong} />
              </motion.div>
            ))}
          </div>
        )}
