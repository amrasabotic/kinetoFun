...
const STEPS: Step[] = [
  ...
  { label: 'Point at the star!', hint: 'Move your fingertip to the middle of the screen', check: (f) => f.detected && Math.abs(f.cursorX - 0.5) < 0.22 && Math.abs(f.cursorY - 0.5) < 0.22 },
];
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-[#2a1a5e] via-[#3a1a6e] to-[#1a0e3d] flex flex-col items-center justify-center text-white overflow-hidden">
      <div className="relative w-[480px] max-w-[86vw] aspect-video rounded-3xl overflow-hidden border-4 border-white/20 shadow-2xl">
        ...
        {stepIndex === 1 && (
          <div className="absolute left-1/2 top-1/2 text-4xl" style={{ transform: 'translate(-50%,-50%)' }}>⭐</div>
        )}
        {frame.detected && (
          <div
