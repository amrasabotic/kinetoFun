...
const DEFAULT_SAVE: SaveData = {
  version: 1,
  bestStarsByMode: { fruit: 0, vegetable: 0, mixed: 0 },
  sessionsPlayed: 0,
  perfectSessions: 0,
  achievements: [],
};
...
export const useProgressStore = create<ProgressState>()(
  persist(
    ...
    { name: 'fruit-vegetable-sorter-progress' },
  ),
);
...
function computeNewAchievements(existing: string[], state: SaveData, result: SessionResult): string[] {
  ...
  if (state.sessionsPlayed >= 10) add('food-champion');
  if (result.mistakes === 0) add('perfect-round');
  if (state.bestStarsByMode.fruit >= 3) add('fruit-expert');
  if (state.bestStarsByMode.vegetable >= 3) add('veggie-expert');
