...
export interface RawHandFrame {
  handCount: number;
  cursorX: number; // index fingertip of the primary hand, mirrored — aims the dart
  cursorY: number;
  isPalmOpen: boolean; // release the thrown dart
  isFist: boolean; // draw back (charge power while held)
...
export function startHandTracking(opts: StartOptions): () => void {
  ...
  async function init() {
    ...
    try {
      ...
      console.error('[gesture-darts] camera/model init failed', err);
