...
import type { BinDef, RoundPrompt, SessionResult, SortMode } from '../../types';
...
export default function GameplayScreen({ mode, onExit, onSessionComplete }: Props) {
  ...
  useEffect(() => {
    const next = generateRound(mode, roundIndex, prevPromptRef.current);
    ...
    if (audioNarration) {
      const text = mode === 'shape' ? shapeLabel(next.prompt.shape)
        : mode === 'color' ? colorLabel(next.prompt.color)
        : `${colorLabel(next.prompt.color)} ${shapeLabel(next.prompt.shape)}`;
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <p className="text-xs text-white/50 uppercase tracking-wide">
            {mode === 'shape' ? 'Sort by Shape' : mode === 'color' ? 'Sort by Color' : 'Mixed Mode'}
          </p>
        ...
        {round && (
          <div className="flex flex-wrap items-center justify-center gap-6 max-w-3xl">
            {round.bins.map((bin) => (
              ...
              >
                <Bin bin={bin} mode={mode} disabled={locked} onCorrect={() => handleCorrect(bin.id)} onWrong={handleWrong} />
              </motion.div>
            ))}
          </div>
        )}
