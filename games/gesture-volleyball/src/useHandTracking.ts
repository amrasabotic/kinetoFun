...
const EMPTY: HandsRaw = { hands: [], handednesses: [], leftDetected: false, rightDetected: false };
...
export function useHandTracking(videoRef: React.RefObject<HTMLVideoElement>): HandsRaw {
  ...
  const detect = useCallback(() => {
    ...
    if (video.currentTime !== lastVideoTime.current) {
      ...
      const handednesses: string[]        = [];
      let leftDetected  = false;
      let rightDetected = false;

      if (results.landmarks?.length > 0) {
        results.landmarks.forEach((landmarks, i) => {
          const h = (results.handednesses[i]?.[0]?.categoryName ?? 'Right') as string;
      ...
      setHandsRaw({ hands, handednesses, leftDetected, rightDetected });
    }
    animFrameRef.current = requestAnimationFrame(detect);
  }, [videoRef]);
