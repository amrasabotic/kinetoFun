...
import type { BinDef, InstrumentGameMode, RoundPrompt, SessionResult } from '../../types';
import { generateRound } from '../../game/roundLogic';
import { finalizeSession, type RoundTally } from '../../game/scoring';
import PromptDisplay from './PromptDisplay';
import InstrumentBin from './InstrumentBin';
...
export default function GameplayScreen({ mode, onExit, onSessionComplete }: Props) {
  ...
  useEffect(() => {
    const next = generateRound(mode, roundIndex, prevInstrumentRef.current);
    prevInstrumentRef.current = next.prompt.instrumentId;
    setRound(next);
  ...
  function handleCorrect(binId: string) {
    ...
    if (audioNarration && round) {
      const instrument = instrumentById(round.prompt.instrumentId);
      if (instrument) {
        window.setTimeout(() => speak(`That's a ${instrument.name}!`, true), 200);
      }
    }
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <p className="text-xs text-white/50 uppercase tracking-wide">
            {mode === 'percussion' ? 'Percussion' : mode === 'melodic' ? 'Melodic' : 'Mixed Mode'}
          </p>
