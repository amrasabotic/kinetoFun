...
export type MainMenuAction = BoardSize | 'settings' | 'exit';

const ITEMS: { action: MainMenuAction; label: string; emoji: string; color: string }[] = [
  { action: 'small', label: 'Small Zoo', emoji: '🐣', color: '#2A5CD6' },
  { action: 'medium', label: 'Medium Zoo', emoji: '🦁', color: '#2FA35A' },
  { action: 'big', label: 'Big Zoo', emoji: '🐘', color: '#F07A26' },
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-[#2a1a5e] via-[#3a1a6e] to-[#1a0e3d] flex flex-col items-center justify-center text-white px-6">
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-4 flex items-center gap-4">
        <div className="text-6xl scs-bob">🎮</div>
        <div>
          ...
          <p className="text-white/50 mt-1 text-lg">Flip cards to match animal pairs!</p>
        </div>
      </motion.div>
        ...
        {ITEMS.map((item, i) => (
          ...
          >
            ...
            >
              ...
              {(item.action === 'small' || item.action === 'medium' || item.action === 'big') && (
