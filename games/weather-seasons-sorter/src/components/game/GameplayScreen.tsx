...
import type { BinDef, WeatherGameMode, RoundPrompt, SessionResult } from '../../types';
import { generateRound } from '../../game/roundLogic';
import { finalizeSession, type RoundTally } from '../../game/scoring';
import PromptDisplay from './PromptDisplay';
import WeatherBin from './WeatherBin';
import HoverButton from '../common/HoverButton';
import { scenarioById, speakableScenario } from '../../data/weather';
...
export default function GameplayScreen({ mode, onExit, onSessionComplete }: Props) {
  ...
  useEffect(() => {
    const next = generateRound(mode, roundIndex, prevScenarioRef.current);
    prevScenarioRef.current = next.prompt.scenarioId;
    setRound(next);
    ...
    if (audioNarration) {
      const scenario = scenarioById(next.prompt.scenarioId);
      if (scenario) {
        const text = speakableScenario(scenario);
  ...
  const binPresentation = round?.prompt.presentation === 'weather' ? 'clothing' : 'weather';
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <p className="text-xs text-white/50 uppercase tracking-wide">
            {mode === 'wear' ? 'What to Wear' : mode === 'match' ? 'Match the Weather' : 'Mixed Mode'}
          </p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-10">
        ...
        <AnimatePresence>
          {wrongThisRound && (
            ...
            >
              Try another match — you can do it!
            </motion.p>
          )}
        </AnimatePresence>
