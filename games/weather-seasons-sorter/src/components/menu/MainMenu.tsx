    ...
    <div className="fixed inset-0 bg-gradient-to-br from-[#2a1a5e] via-[#3a1a6e] to-[#1a0e3d] flex flex-col items-center justify-center text-white px-6">
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-4 flex items-center gap-4">
        <div className="scs-bob">
          <WeatherIcon weather="sunny" size={72} />
        </div>
        <div>
          <h1 className="text-5xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-sky-200 via-white to-amber-200">
            Weather & Seasons Sorter
          </h1>
          <p className="text-white/50 mt-1 text-lg">Learn what to wear!</p>
        </div>
      </motion.div>
        ...
        {ITEMS.map((item, i) => (
          ...
          >
            ...
            >
              ...
              {(item.action === 'wear' || item.action === 'match' || item.action === 'mixed') && (
      ...
      <p className="mt-8 text-sm text-white/50 max-w-md text-center">
        A weather scene or clothing item appears — hover your fingertip over the matching answer for about a second to select it.
      </p>
