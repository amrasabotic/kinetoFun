...
// Thumb-tip-to-index-tip distance threshold (normalized coords), tuned identically to
// the pinch-drag mechanic proven in games/gesture-love-balls.
...
function classify(lm: NormalizedLandmark[], mirror: boolean): { cursorX: number; cursorY: number; indexZ: number; isPalmOpen: boolean; isFist: boolean; isPinching: boolean } {
  const wrist = lm[0];
  const midMcp = lm[9];
  ...
  return {
    ...
    isFist: spread < 0.55,
    isPinching: dist(lm[4], lm[8]) < PINCH_THRESHOLD,
  };
}
...
export function startHandTracking(opts: StartOptions): () => void {
  ...
  async function init() {
    ...
    try {
      ...
      console.error('[farm-builder] camera/model init failed', err);
