...
const DEFAULT_SAVE: SaveData = {
  version: 1,
  bestStarsByMode: { read: 0, set: 0, mixed: 0 },
  sessionsPlayed: 0,
  perfectSessions: 0,
  achievements: [],
};
...
export const useProgressStore = create<ProgressState>()(
  persist(
    ...
    { name: 'clock-time-teller-progress' },
  ),
);
...
function computeNewAchievements(existing: string[], state: SaveData, result: SessionResult): string[] {
  ...
  if (state.bestStarsByMode.read >= 3) add('clock-reader-master');
  if (state.bestStarsByMode.set >= 3) add('clock-setter-master');
