    ...
    <div className="fixed inset-0 bg-gradient-to-br from-[#2a1a5e] via-[#3a1a6e] to-[#1a0e3d] flex flex-col items-center justify-center text-white px-6">
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-4 flex items-center gap-4">
        <div className="scs-bob">
          <ClockFace time={{ hour: 10, minute: 15 }} size={72} />
        </div>
        <div>
          ...
          <p className="text-white/50 mt-1 text-lg">Learn to tell time!</p>
        </div>
      </motion.div>
        ...
        {ITEMS.map((item, i) => (
          ...
          >
            ...
            >
              ...
              {(item.action === 'read' || item.action === 'set' || item.action === 'mixed') && (
      ...
      <p className="mt-8 text-sm text-white/50 max-w-md text-center">
        A clock shows a time one way — hover your fingertip over the matching answer for about a second to select it.
      </p>
