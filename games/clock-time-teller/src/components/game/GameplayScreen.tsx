...
import type { BinDef, ClockGameMode, RoundPrompt, SessionResult } from '../../types';
import { generateRound } from '../../game/roundLogic';
import { finalizeSession, type RoundTally } from '../../game/scoring';
import PromptDisplay from './PromptDisplay';
import TimeBin from './TimeBin';
import HoverButton from '../common/HoverButton';
import { speakableTime } from '../../data/times';
...
export default function GameplayScreen({ mode, onExit, onSessionComplete }: Props) {
  ...
  const prevTimeRef = useRef<RoundPrompt['time'] | null>(null);

  useEffect(() => {
    const next = generateRound(mode, roundIndex, prevTimeRef.current);
    prevTimeRef.current = next.prompt.time;
    setRound(next);
    ...
    if (audioNarration) {
      const text = speakableTime(next.prompt.time);
  ...
  const binPresentation = round?.prompt.presentation === 'clock' ? 'digital' : 'clock';
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <p className="text-xs text-white/50 uppercase tracking-wide">
            {mode === 'read' ? 'Read the Clock' : mode === 'set' ? 'Set the Clock' : 'Mixed Mode'}
          </p>
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-10">
        ...
        <AnimatePresence>
          {wrongThisRound && (
            ...
            >
              Try another time — you can do it!
            </motion.p>
          )}
        </AnimatePresence>
